...
usersApiRouter.get('/', asyncHandler(async (_req, res) => {
...
usersApiRouter.get('/:id', validateParams(userIdSchema), asyncHandler(async (req, res) => {
    const { id } = req.params;
...
usersApiRouter.post('/', validateRequest(createUserSchema), asyncHandler(async (req, res) => {
    ...
    await newUser.save();
    Logger.info('Utilisateur créé avec succès', { userId: String(newUser._id), email });
    ApiResponseUtil.success(res, 201, sanitizeUser(newUser));
}));
// PATCH /api/users/:id - Mise à jour partielle d'un utilisateur
usersApiRouter.patch('/:id', validateParams(userIdSchema), validateRequest(updateUserSchema), asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    const updatedUser = await User.findByIdAndUpdate(id, { ...updateData, updatedAt: new Date() }, { new: true, runValidators: true }).select('-passwordHash');
    if (!updatedUser) {
        return ApiResponseUtil.notFound(res, 'Utilisateur non trouvé');
    }
...
usersApiRouter.delete('/:id', validateParams(userIdSchema), asyncHandler(async (req, res) => {
    const { id } = req.params;
    // Vérifier si l'utilisateur existe
    ...
    const deletedUser = await User.findByIdAndUpdate(id, { isActive: false, updatedAt: new Date() }, { new: true }).select('-passwordHash');
    if (!deletedUser) {
        return ApiResponseUtil.notFound(res, 'Utilisateur non trouvé');
    }
