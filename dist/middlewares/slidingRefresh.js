...
export const slidingRefresh = (secretEnv) => async (req, res, next) => {
    try {
        const oldRefresh = req.cookies?.refreshToken;
        if (!oldRefresh)
            return next();
        const payload = jwt.verify(oldRefresh, process.env[secretEnv]);
        const now = Math.floor(Date.now() / 1000);
        const ONE_DAY = 24 * 60 * 60;
        if (payload.exp - now < 6 * ONE_DAY) {
            const newRefresh = jwt.sign({ userId: payload.userId, role: payload.role }, process.env[secretEnv], { expiresIn: '7d' });
...
//   /* … suite logique SQL … */
// });
//# sourceMappingURL=slidingRefresh.js.map